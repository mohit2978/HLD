...
Low latency requires async replication. To respond quickly, accept the write locally and replicate asynchronously. The client sees fast response times. But other nodes lag behind, providing weaker consistency. `low latency means evenetual consistentcy`.
...
Fault---> i did not studey 

failure--> i failed in exam

fault leads to failure

Fault tolerant--> not going to fail even after fault

Resiliant--> after failure recover quickly

fault tolerant is state before failure and Resiliant is state after failure
